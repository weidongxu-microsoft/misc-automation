        ...
        for entry in resource_manager_dict.items():
            ...
            for entry1 in resource_version_dict.items():
                ...
                for item in resource_type_array:
                    ...
                    spec_path = item["path"]
                    spec_info_collection_dict[resource_provider].spec_info_list.append(
                        SpecInfo(resource_type, resource_version, spec_path)
                    )
...
def generate_arm_code(spec_tag_list: List[SpecTag]):
    ...
    for spec_tag in spec_tag_list:
        code_path = os.path.join(
            SDK_OUTPUT_PATH, "src/main/java/com/azure/resourcemanager", SDK_AZS_NAMESPACE, spec_tag.sdk_namespace
        )
    ...
    for manager in manager_list:
        print(
            (
                f"/** @return the {{@link {manager.class_name}}}. */\n"
    ...
    for manager in manager_list:
        print(
            f"{manager.class_name}.Configurable {manager.variable_name}Configurable = {manager.class_name}.configure();"
        )
    print()
    print(
